...
<!--
        ...
        const tapObserverValues = [
        { name: "1) subscribe", value: "/" },
        { name: "2) next", value: "/" },
        { name: "2) error", value: "/" },
        { name: "3) complete", value: "/" },
        { name: "3) unsubscribe", value: "/" },
        { name: "4) finalize", value: "/" },
    ];
    ...
    .pipe(
        delay(2000),
        tap({
            // specific tap observer function
            subscribe: () => (tapObserverValues[0].value = "The subscription started"),
        }),
        delay(1000),
        tap({
            // next: (value) => (tapObserverValues[1].value = value.name),
            next: (value) => {
                throw new Error();
            },
        }),
        delay(1000),
        tap({
            error: (err) => (tapObserverValues[2].value = err),
            complete: () => (tapObserverValues[3].value = "completed"),
            unsubscribe: () => (tapObserverValues[4].value = "unsubscribed manually"),
        }),
        delay(2000),
        tap({
            finalize: () => (tapObserverValues[5].value = "The subscription was destroyed"),
        })
    )
    .subscribe(getFullObserver("(final)"));

        <section class="border-4 p-8 m-4 flex flex-col">
            <h1 class="text-3xl font-bold flex justify-start mb-4 text-gray-400">3. Tap Observer</h1>
            <div class="flex basis-1/3 w-full flex-wrap justify-center">
                {#each tapObserverValues as tapObserverValue}
                    <div class="bg-purple-200 rounded-md p-4 mr-4 my-4 flex flex-col w-48 h-32">
                        <h2 class="text-xl font-bold mb-1">{tapObserverValue.name}</h2>
                        <h3 class="text-gray-400">{tapObserverValue.value}</h3>
                    </div>
                {/each}
            </div>
        </section> -->
